...
use crate::error_handling::{display_success, display_warning, display_info};
...
impl Config {
    ...
    fn validate_config(config: &Config) -> Result<()> {
        ...
        if config.context_timeout > 60000 {
            return Err(anyhow!("context_timeout cannot exceed 60 seconds (60000ms)"));
        }
    ...
    pub fn set_auto_execute(&mut self, enabled: bool) -> Result<()> {
        ...
        if enabled {
            ...
            display_success("Auto-execution disabled. Commands will be displayed for manual execution.");
        }
    ...
    pub fn set_dry_run(&mut self, enabled: bool) -> Result<()> {
        ...
        if enabled {
            ...
            display_success("Dry-run mode disabled. Commands can be executed based on auto_execute setting.");
        }
    ...
    pub fn set_safety_level(&mut self, level: SafetyLevel) -> Result<()> {
        ...
        match level {
            SafetyLevel::Low => display_warning("Safety level set to LOW. Fewer warnings will be shown."),
            SafetyLevel::Medium => display_success("Safety level set to MEDIUM. Balanced safety checks."),
            SafetyLevel::High => display_success("Safety level set to HIGH. Maximum safety checks enabled."),
        }
    ...
    pub fn set_context_timeout(&mut self, timeout_ms: u64) -> Result<()> {
        ...
        if timeout_ms > 60000 {
            return Err(anyhow!("Context timeout cannot exceed 60 seconds (60000ms)"));
        }
    ...
    pub fn display(&self) {
        ...
        println!("Auto-execute: {}", if self.auto_execute { "enabled ⚠️" } else { "disabled 🛡️" });
        println!("Dry-run mode: {}", if self.dry_run { "enabled 🔍" } else { "disabled" });
        ...
        println!("Cloud Mode: {}", if self.use_cloud { "enabled ☁️" } else { "disabled 🏠" });
        println!("Backend URL: {}", self.backend_url);
        println!("API Token: {}", if self.api_token.is_some() { "********" } else { "none" });
    }
...
mod tests {
    ...
    fn test_config_validation_zero_timeout() {
        ...
        assert!(result.unwrap_err().to_string().contains("context_timeout must be greater than 0"));
    }
    ...
    fn test_config_validation_excessive_timeout() {
        ...
        let result = Config::validate_config(&config);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("context_timeout cannot exceed 60 seconds"));
    }
    ...
    fn test_config_validation_invalid_url() {
        ...
        let result = Config::validate_config(&config);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("ollama_url must be a valid HTTP/HTTPS URL"));
    }
