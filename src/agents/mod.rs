...
use crate::history::{ContextPriority, ContextWindow, History};
use crate::intent::{IntentAnalysis, IntentClassifier, UserIntent};
use crate::os_context::OSContext;
use crate::performance::{OperationType, PerformanceMonitor, SystemPerformanceSummary};
use crate::providers::{
    CircuitBreakerState, CloudProvider, OllamaProvider, ProviderManager, ProviderType,
};
...
impl Orchestrator {
    pub fn new(config: Config, history: History) -> Self {
        ...
        let ollama_provider =
            OllamaProvider::with_timeout(config.ollama_url.clone(), config.model.clone(), timeout);
    ...
    pub async fn process(&mut self, prompt: &str) -> Result<String> {
        ...
        self.log_activity(&format!(
            "Intent Analysis: {:?} (confidence: {:.2}) - took {}ms",
            intent_analysis.intent,
            intent_analysis.confidence,
            intent_duration.as_millis()
        ));
        ...
        if self.intent_classifier.is_destructive_action(prompt) {
            ...
            if intent_analysis.intent == UserIntent::Explanatory {
                return Ok("Command: (none)\n\nI can explain how destructive operations work, but I won't suggest actual destructive commands for explanatory requests. Please clarify if you want to learn about these operations or actually perform them.".to_string());
                ...
                self.log_activity(
                    "Proceeding with destructive actionable request (will be flagged as sensitive)",
                );
                self.show_work(
                    "Proceeding with destructive request - will be flagged as sensitive",
                );
            }
        }
        ...
        if let Some(builtin_cmd) = self.builtin_commands.match_command(prompt) {
            ...
            self.log_activity(&format!(
                "Built-in Command Matched: {} - took {}ms",
                builtin_cmd.description,
                builtin_match_duration.as_millis()
            ));
            self.show_work(&format!(
                "Built-in command matched: {} ({}ms)",
                builtin_cmd.description,
                builtin_match_duration.as_millis()
            ));
            ...
            if let Err(validation_error) = self
                .intent_classifier
                .validate_command_for_intent(&command, &intent_analysis.intent)
            {
                self.log_activity(&format!(
                    "Built-in command validation failed: {}",
                    validation_error
                ));
            ...
            self.provider_manager
                .get_performance_monitor_mut()
                .record_measurement(OperationType::BuiltinCommand, builtin_duration, true);

            self.log_activity(&format!(
                "Built-in Response: {} - total time {}ms",
                response
                    .replace('\n', " ")
                    .chars()
                    .take(100)
            ...
            self.show_work(&format!(
                "Built-in response generated ({}ms)",
                builtin_duration.as_millis()
            ));
            return Ok(response);
        }
        ...
        self.log_activity(&format!(
            "Detected Category: {} - analysis took {}ms",
            category,
            ...
            for cmd_val in cmds {
                if let Some(cmd) = cmd_val.as_str() {
                    ...
                    let context_cmd = match cmd {
                        ...
                        cmd if cmd.starts_with("cat /etc/os-release") => {
                            Some("os-release".to_string())
                        }
        ...
        let system_context = self
            .context_gatherer
            .gather_context(&context_commands)
            .await;
        ...
        self.provider_manager
            .get_performance_monitor_mut()
            .record_measurement(OperationType::ContextGathering, context_duration, true);

        let context_str = self
            .context_gatherer
            .format_context_for_prompt(&system_context);

        if !context_str.is_empty() {
            self.log_activity(&format!(
                "Gathered system context in {}ms - {} chars",
                context_duration.as_millis(),
                context_str.len()
            ));
            self.show_work(&format!(
                "Context gathered: {} chars ({}ms)",
                context_str.len(),
                context_duration.as_millis()
            ));
        } else {
        ...
        let final_prompt = self.build_agent_prompt_with_intent(
            agent,
            prompt,
            &context_window,
            &context_str,
        ...
        self.log_activity(&format!(
            "Built final prompt: {} chars - took {}ms",
            final_prompt.len(),
            prompt_build_duration.as_millis()
        ));
        self.show_work(&format!(
            "Final prompt built: {} chars ({}ms)",
            final_prompt.len(),
            prompt_build_duration.as_millis()
        ));
        ...
        let response = if category == "SHELL" {
            ...
            self.show_work(&format!(
                "Initial AI response: {} chars ({}ms)",
                initial_response.len(),
                ai_initial_duration.as_millis()
            ));

            let validation_start = Instant::now();
            let validated_response = self
                .validate_and_retry_shell_response(prompt, &initial_response, 2)
                .await?;
            let validation_duration = validation_start.elapsed();
            self.show_work(&format!(
                "Response validation completed ({}ms)",
            ...
            if let Some(command) = extract_command(&validated_response) {
                ...
                {
                    self.log_activity(&format!(
                        "Generated command validation failed: {}",
                        validation_error
                    ));
                    self.show_work(&format!(
                        "Command-intent validation failed: {}",
                        validation_error
                    ));
                    return Ok(format!(
                        "Command: (none)\n\n{}\n\nOriginal request: {}",
                        validation_error, prompt
                    ));
                }
            }
        ...
        self.log_activity(&format!(
            "AI Response: {} - length: {} chars - took {}ms",
            response
                .replace('\n', " ")
                .chars()
                .take(100)
                .collect::<String>(),
            response.len(),
            total_ai_duration.as_millis()
        ));
        self.show_work(&format!(
            "AI response generated: {} chars ({}ms)",
            response.len(),
            total_ai_duration.as_millis()
        ));
    ...
    fn build_agent_prompt_with_intent(
        &self,
        agent: &AgentProfile,
        user_prompt: &str,
        context_window: &ContextWindow,
        ...
        if agent.name == "ShellExpert" {
            ...
            match intent_analysis.intent {
                UserIntent::Explanatory => {
                    ...
                    full_prompt
                        .push_str("- Use 'Command: (none)' and provide educational explanation\n");
                    full_prompt.push_str(
                        "- If you must show a command example, make it clearly educational\n",
                    );
                    full_prompt
                        .push_str("- Do NOT suggest destructive or system-modifying commands\n");
    ...
    async fn call_ollama_with_prompt(
        &mut self,
        full_prompt: &str,
        agent: &AgentProfile,
    ) -> Result<String> {
    ...
    async fn validate_and_retry_shell_response(
        &mut self,
        original_prompt: &str,
        response: &str,
        max_retries: u32,
        ...
        if max_retries > 0 {
            self.log_activity(&format!(
                "ShellExpert response format invalid, retrying. Attempts left: {}",
                max_retries
            ));
...
Original request: {}",
            ...
            let full_retry_prompt =
                self.build_agent_prompt(&SHELL_EXPERT, &retry_prompt, &context_window, "");

            let retry_response = self
                .call_ollama_with_prompt(&full_retry_prompt, &SHELL_EXPERT)
                .await?;

            // Use Box::pin to handle async recursion
            return Box::pin(self.validate_and_retry_shell_response(
                original_prompt,
                &retry_response,
                max_retries - 1,
            ))
    ...
    pub async fn is_local_provider_available(&self) -> bool {
        if let Some(provider) = self
            .provider_manager
            .get_provider_by_type(&ProviderType::Local)
        {
    ...
    pub async fn is_cloud_provider_available(&self) -> bool {
        if let Some(provider) = self
            .provider_manager
            .get_provider_by_type(&ProviderType::Cloud)
        {
            provider.is_available().await
        } else {
            false
    ...
    fn find_builtin_command_id(
        &self,
        target_cmd: &crate::builtin_commands::BuiltinCommand,
    ) -> String {
    ...
    async fn call_ollama_no_history(
        &mut self,
        agent: &AgentProfile,
        prompt: &str,
    ) -> Result<String> {
    ...
    async fn execute_ollama_call(
        ...
        match self
            .provider_manager
            .get_response(full_prompt, agent, timeout)
            .await
        {
...
fn is_shell_command(cmd: &str) -> bool {
    let common_commands = [
        ...
        "ping", "curl", "wget", "df", "du", "free", "top", "htop", "whoami", "hostname", "uptime",
