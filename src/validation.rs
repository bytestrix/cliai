...
impl ShellToken {
    ...
    pub fn is_quoted(&self) -> bool {
        matches!(
            self,
            ShellToken::SingleQuoted(_) | ShellToken::DoubleQuoted(_)
        )
    }
...
impl ShellParser {
    ...
    pub fn parse(&self, command: &str) -> Result<Vec<ShellToken>> {
        ...
        if !current_token.is_empty() {
            if in_single_quote {
                // For unclosed single quotes, treat as unquoted token with warning
                tokens.push(ShellToken::Unquoted(format!("'{}", current_token)));
            } else if in_double_quote {
                // For unclosed double quotes, treat as unquoted token with warning
                tokens.push(ShellToken::Unquoted(format!("\"{}", current_token)));
            } else {
    ...
    pub fn new() -> Self {
        let sensitive_patterns = vec![
            ...
            SensitivePattern {
                ...
                description: "Fork bomb detected - this will consume all system resources"
                    .to_string(),
            ...
            SensitivePattern {
                ...
                severity: SeverityLevel::Dangerous,
                description: "mkfs command creates new filesystem, destroying existing data"
                    .to_string(),
...
pub trait CommandValidator {
    ...
    /// Check if a string looks like a shell command vs natural language
...
impl DefaultCommandValidator {
    ...
    fn is_dangerous(&self, command: &str) -> Vec<SecurityWarning> {
        ...
        match safety_result {
            ...
            SafetyResult::Warning(warnings) => warnings
                .into_iter()
                .map(|(severity, msg)| match severity {
                ...
                .collect(),
            SafetyResult::RequiresConfirmation(warnings) => warnings
                .into_iter()
                .map(|(severity, msg)| match severity {
                    SeverityLevel::Warning => SecurityWarning::DangerousPattern(msg),
                    SeverityLevel::Dangerous => SecurityWarning::DataLoss(msg),
                ...
                .collect(),
            SafetyResult::Blocked(warnings) => warnings
                .into_iter()
                .map(|(severity, msg)| match severity {
                    SeverityLevel::Warning => SecurityWarning::DangerousPattern(msg),
                    SeverityLevel::Dangerous => SecurityWarning::DataLoss(msg),
    ...
    fn standardize_file_existence_checks(&self, command: &str) -> String {
        ...
        if result.contains("test -f ") && !result.contains("&& echo 'exists' || echo 'not found'") {
            ...
            let test_f_pattern =
                Regex::new(r#"\btest\s+-f\s+('[^']*'|"[^"]*"|[^\s&|;]+)"#).unwrap();
        ...
        if result.contains("test -d ") && !result.contains("&& echo 'exists' || echo 'not found'") {
            let test_d_pattern =
                Regex::new(r#"\btest\s+-d\s+('[^']*'|"[^"]*"|[^\s&|;]+)"#).unwrap();
        ...
        if stat_pattern.is_match(&result) && !result.contains("&&") {
            result = stat_pattern
                .replace_all(&result, |caps: &regex::Captures| {
                    let filename = &caps[1];
                    format!(
                        "stat {} >/dev/null 2>&1 && echo 'exists' || echo 'not found'",
                        filename
                    )
                })
                .to_string();
    ...
    fn fix_unquoted_paths_in_test_commands(&self, command: &str) -> String {
        ...
        let test_with_spaces_pattern =
            Regex::new(r#"\b(test\s+-[fd])\s+([^'"&|;]+(?:\s+[^'"&|;]+)+)"#).unwrap();

        let mut result = test_with_spaces_pattern
        ...
        let stat_with_spaces_pattern =
            Regex::new(r#"\b(stat)\s+([^'"&|;]+(?:\s+[^'"&|;]+)+)"#).unwrap();
        result = stat_with_spaces_pattern
            .replace_all(&result, |caps: &regex::Captures| {
        ...
        result = test_single_word_pattern
            .replace_all(&result, |caps: &regex::Captures| {
                let command_part = &caps[1]; // "test -f" or "test -d"
        ...
        result = stat_single_word_pattern
            .replace_all(&result, |caps: &regex::Captures| {
                let command_part = &caps[1]; // "stat"
                let path_part = &caps[2]; // "file$var.txt"
...
impl CommandValidator for DefaultCommandValidator {
    fn validate(&self, command: &str) -> ValidationResult {
        ...
        // Skip most validation for natural language queries
        ...
        if quoting_analysis.needs_correction {
            ...
            if has_serious_issues {
                let error_messages: Vec<String> = quoting_analysis
                    .issues_found
                    .iter()
        ...
        // Check basic quoting syntax (unclosed quotes, etc.) - now more lenient
    ...
    fn check_quoting(&self, command: &str) -> Result<()> {
        // Skip quoting validation for natural language queries that aren't shell commands
        ...
        // For malformed quotes, issue a warning but don't block execution
        if in_single_quote || in_double_quote {
            // Return Ok() but log the issue - don't block execution
            return Ok(());
        }
    ...
    fn looks_like_shell_command(&self, text: &str) -> bool {
        ...
        // If it starts with common shell commands, it's likely a command
        let shell_command_prefixes = [
            "ls", "cd", "pwd", "mkdir", "rmdir", "rm", "cp", "mv", "cat", "grep", "find", "chmod",
            "chown", "sudo", "git", "npm", "yarn", "cargo", "echo", "touch", "head", "tail",
            "sort", "uniq", "wc", "awk", "sed",
        ];

        for prefix in &shell_command_prefixes {
        ...
        // If it contains shell operators, it's likely a command
        if text.contains(" | ")
            || text.contains(" && ")
            || text.contains(" || ")
            || text.contains(" > ")
        ...
        // If it starts with a quote and looks malformed, it might be user input error
        if (text.starts_with('"') && !text.ends_with('"'))
            || (text.starts_with('\'') && !text.ends_with('\''))
        {
            return false; // Treat as natural language, not a shell command
        }
...
mod tests {
    ...
    fn test_shell_parser_quoted_strings() {
        ...
        assert_eq!(
            tokens[2],
            ShellToken::DoubleQuoted("test string".to_string())
        );
    }
    ...
    fn test_enhanced_fork_bomb_patterns() {
        ...
        let fork_bombs = vec![":(){ :|:& };:", ":(){:|:&};:", "bomb(){ bomb|bomb& };bomb"];
    ...
    fn test_debug_stat_command() {
        ...
        match result {
            ...
            ValidationResult::Rewritten(cmd, fixes) => {
                println!("Rewritten: {} with fixes: {:?}", cmd, fixes)
            }
            ValidationResult::Invalid(cmd, errors) => {
                println!("Invalid: {} with errors: {:?}", cmd, errors)
            }
            ValidationResult::Sensitive(cmd, warnings) => {
                println!("Sensitive: {} with warnings: {:?}", cmd, warnings)
            }
        }
    }
    ...
    fn test_stat_command_standardization() {
        ...
        match result {
            ValidationResult::Rewritten(cmd, fixes) => {
                assert_eq!(
                    cmd,
                    "stat myfile.txt >/dev/null 2>&1 && echo 'exists' || echo 'not found'"
                );
                assert!(fixes.contains(&"Standardized file existence check format".to_string()));
            }
        ...
        match result {
            ValidationResult::Rewritten(cmd, fixes) => {
                assert_eq!(
                    cmd,
                    "stat 'my file.txt' >/dev/null 2>&1 && echo 'exists' || echo 'not found'"
                );
                assert!(fixes.contains(&"Standardized file existence check format".to_string()));
            }
    ...
    fn test_special_characters_quoting() {
        ...
        for (input_file, expected_quoted) in special_files {
            ...
            match result {
                ...
                ValidationResult::Valid(cmd) => {
                    ...
                    assert!(
                        cmd.contains(input_file),
                        "Command should contain the filename: {}",
                        input_file
                    );
                }
                _ => panic!(
                    "Expected rewritten or valid result for file with special chars: {}",
                    input_file
                ),
            }
        }
        ...
        for input_file in shell_operator_files {
            ...
            match result {
                ...
                _ => panic!(
                    "Expected rewritten or valid result for pre-quoted file: {}",
                    input_file
                ),
            }
        }
    ...
    fn test_single_quotes_in_path() {
        ...
        match result {
            ...
            ValidationResult::Invalid(_, _) => {
                ...
                println!(
                    "Command was marked as invalid, which is acceptable for unquoted single quotes"
                );
            }
    ...
    fn test_multiple_file_existence_checks() {
        ...
        match result {
            ValidationResult::Rewritten(cmd, _) => {
                ...
                assert_eq!(
                    cmd,
                    "test -f file1.txt && echo 'exists' || echo 'not found' && test -f file2.txt"
                );
            }
        ...
        match result1 {
            ValidationResult::Rewritten(cmd, _) => {
                assert_eq!(
                    cmd,
                    "test -f file1.txt && echo 'exists' || echo 'not found'"
                );
            }
    ...
    fn test_no_double_standardization() {
        ...
        let result = validator
            .validate("stat myfile.txt >/dev/null 2>&1 && echo 'exists' || echo 'not found'");
        match result {
            ValidationResult::Valid(validated_cmd) => {
                assert_eq!(
                    validated_cmd,
            ...
            _ => panic!(
                "Stat command with existing format should be valid or rewritten consistently"
            ),
        }
    }
    ...
    fn test_path_quoting_validation() {
        ...
        let result =
            validator.validate("test -f 'my file.txt' && echo 'exists' || echo 'not found'");
        match result {
            ValidationResult::Valid(_) => {} // Expected
...
mod format_tests {
    ...
    fn test_valid_shell_response_format() {
        ...
        for response in valid_responses {
            assert!(
                orchestrator.is_valid_shell_response(response),
    ...
    fn test_valid_command_line() {
        ...
        for cmd in valid_commands {
            assert!(
                orchestrator.is_valid_command_line(cmd),
    ...
    fn test_extract_command_with_new_format() {
        ...
        assert_eq!(
            extract_command("Command: ls -la"),
            Some("ls -la".to_string())
        );
        assert_eq!(
            extract_command("Command: find . -name '*.rs' | wc -l"),
            Some("find . -name '*.rs' | wc -l".to_string())
        );

        // "(none)" case should return None
        assert_eq!(
            extract_command("Command: (none)\nThis is an explanation."),
            None
        );
        ...
        assert_eq!(
            extract_command("🚀 Executing: ls -la"),
            Some("ls -la".to_string())
        );
