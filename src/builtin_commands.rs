...
impl BuiltinCommands {
    ...
    pub fn new() -> Self {
        ...
        commands.insert("current_directory".to_string(), BuiltinCommand {
        ...
        commands.insert("make_directory".to_string(), BuiltinCommand {
        ...
        commands.insert("create_file".to_string(), BuiltinCommand {
        ...
        commands.insert("show_file".to_string(), BuiltinCommand {
        ...
        commands.insert("count_lines".to_string(), BuiltinCommand {
        ...
        commands.insert("find_files".to_string(), BuiltinCommand {
        ...
        commands.insert("file_exists".to_string(), BuiltinCommand {
        ...
        commands.insert("current_user".to_string(), BuiltinCommand {
        ...
        commands.insert("hostname".to_string(), BuiltinCommand {
        ...
        commands.insert("system_info".to_string(), BuiltinCommand {
            pattern: Regex::new(r"^(uname -a|system info|system information|os info)$").unwrap(),
        ...
        commands.insert("disk_usage".to_string(), BuiltinCommand {
        ...
        commands.insert("memory_usage".to_string(), BuiltinCommand {
        ...
        commands.insert("find_command".to_string(), BuiltinCommand {
            pattern: Regex::new(r"^(which|find command|locate command|command location)$").unwrap(),
        ...
        commands.insert("list_processes".to_string(), BuiltinCommand {
            pattern: Regex::new(r"^(ps aux|list processes|show processes|running processes)$").unwrap(),
        ...
        commands.insert("make_executable".to_string(), BuiltinCommand {
            pattern: Regex::new(r"^(chmod \+x|make executable|executable permission)$").unwrap(),
        ...
        commands.insert("git_status".to_string(), BuiltinCommand {
        ...
        commands.insert("git_log".to_string(), BuiltinCommand {
        ...
        commands.insert("ping_test".to_string(), BuiltinCommand {
        ...
        commands.insert("download_file".to_string(), BuiltinCommand {
    ...
    pub fn match_command(&self, input: &str) -> Option<&BuiltinCommand> {
        ...
        if (trimmed.contains("folder") || trimmed.contains("directory")) && 
           (trimmed.contains("inside") || trimmed.contains("nested") || 
            trimmed.contains("multiple") || trimmed.contains("10") || 
            trimmed.contains("several") || trimmed.contains("many") ||
            trimmed.contains("and")) {
            return None; // Let AI handle complex folder structures
        }
        
        // Try to match against all command patterns
        for (_id, command) in &self.commands {
    ...
    pub fn get_categories(&self) -> Vec<CommandCategory> {
        let mut categories: Vec<CommandCategory> = self.commands
            .values()
    ...
    pub fn generate_command(&self, builtin_cmd: &BuiltinCommand, user_input: &str) -> String {
        ...
        match template.as_str() {
            "mkdir" => {
                ...
                if let Some(dir_name) = self.extract_argument_after_keywords(user_input, &["mkdir", "directory"]) {
            ...
            "touch" => {
                ...
                if let Some(filename) = self.extract_argument_after_keywords(user_input, &["touch", "file"]) {
            ...
            "cat" => {
                ...
                if let Some(filename) = self.extract_argument_after_keywords(user_input, &["cat", "file"]) {
                    format!("cat {}", filename)
                } else {
                    format!("{} <filename>", template)
                }
            ...
            "wc -l" => {
                ...
                if let Some(filename) = self.extract_argument_after_keywords(user_input, &["wc", "lines", "count"]) {
            ...
            "find . -name" => {
                ...
                if let Some(pattern) = self.extract_argument_after_keywords(user_input, &["find", "search", "files"]) {
            ...
            "test -f" => {
                ...
                if let Some(filename) = self.extract_argument_after_keywords(user_input, &["test", "file", "exists", "check"]) {
                    let quoted_filename = self.quote_path_if_needed(&filename);
                    format!("test -f {} && echo 'exists' || echo 'not found'", quoted_filename)
                } else {
                    format!("{} <filename> && echo 'exists' || echo 'not found'", template)
                }
            }
            "test -d" => {
                // Extract directory name and create full existence check with proper quoting
                if let Some(dirname) = self.extract_argument_after_keywords(user_input, &["test", "directory", "dir", "exists", "check"]) {
                    let quoted_dirname = self.quote_path_if_needed(&dirname);
                    format!("test -d {} && echo 'exists' || echo 'not found'", quoted_dirname)
                } else {
                    format!("{} <directory_name> && echo 'exists' || echo 'not found'", template)
                }
            }
            "which" => {
                // Extract command name from input like "which ls" or "find command ls"
                if let Some(cmd_name) = self.extract_argument_after_keywords(user_input, &["which", "command"]) {
            ...
            "chmod +x" => {
                ...
                if let Some(filename) = self.extract_argument_after_keywords(user_input, &["chmod", "executable", "file"]) {
            ...
            "ping -c 4" => {
                ...
                if let Some(host) = self.extract_argument_after_keywords(user_input, &["ping", "connection", "test"]) {
        ...
        for keyword in keywords {
            if let Some(pos) = input_lower.find(keyword) {
                ...
                for word in words {
                    ...
                    if ["the", "a", "an", "to", "for", "in", "on", "at", "by"].contains(&word_lower.as_str()) {
                        continue;
                    }
                    
                    // Skip the word "file" or "directory" if it appears after the keyword
                    if ["file", "directory", "dir", "files"].contains(&word_lower.as_str()) && meaningful_words.is_empty() {
                        continue;
                    }
    ...
    pub fn get_arch_commands(&self) -> Option<String> {
        if let Some(arch_commands) = self.os_context.get_arch_specific_commands() {
            Some(format!(
                ...
                arch_commands.aur_helper.as_deref().unwrap_or("Not detected (install yay or paru)")
            ))
        } else {
            None
        }
...
mod tests {
    ...
    fn test_builtin_commands_creation() {
        ...
        assert_eq!(builtin.count(), 21, "Should have exactly 21 built-in commands (20 original + 1 directory_exists)");
    }
    ...
    fn test_complex_input_rejection() {
        ...
        assert!(builtin.match_command("how do I list files in a directory").is_none());
        assert!(builtin.match_command("what is the current directory and show me files").is_none());
        assert!(builtin.match_command("explain how to use ls command").is_none());
        assert!(builtin.match_command("help me understand git status").is_none());
    ...
    fn test_file_and_directory_existence_commands() {
        ...
        if let Some(cmd) = builtin.match_command("file exists") {
            ...
            assert_eq!(generated, "test -f test.txt && echo 'exists' || echo 'not found'");
        }
        ...
        if let Some(cmd) = builtin.match_command("directory exists") {
            ...
            assert_eq!(generated, "test -d test_dir && echo 'exists' || echo 'not found'");
        }
        ...
        if let Some(cmd) = builtin.match_command("dir exists") {
            ...
            assert_eq!(generated, "test -d my_directory && echo 'exists' || echo 'not found'");
        }
    }
    ...
    fn test_path_quoting() {
        ...
        if let Some(cmd) = builtin.match_command("file exists") {
            ...
            assert_eq!(generated, "test -f 'my file.txt' && echo 'exists' || echo 'not found'");
        }
        
        // Test directory with spaces
        if let Some(cmd) = builtin.match_command("directory exists") {
            let generated = builtin.generate_command(cmd, "check directory my directory");
            assert_eq!(generated, "test -d 'my directory' && echo 'exists' || echo 'not found'");
        }
        
        // Test file with special characters
        if let Some(cmd) = builtin.match_command("file exists") {
            let generated = builtin.generate_command(cmd, "check file file$with*special.txt");
            assert_eq!(generated, "test -f 'file$with*special.txt' && echo 'exists' || echo 'not found'");
        }
    }
    ...
    fn test_quote_path_if_needed() {
        ...
        // No quoting needed
        assert_eq!(builtin.quote_path_if_needed("simple_file.txt"), "simple_file.txt");
        ...
        assert_eq!(builtin.quote_path_if_needed("file with spaces"), "'file with spaces'");
        
        // Quoting needed for special characters
        assert_eq!(builtin.quote_path_if_needed("file$var"), "'file$var'");
        assert_eq!(builtin.quote_path_if_needed("file*pattern"), "'file*pattern'");
        assert_eq!(builtin.quote_path_if_needed("file[123]"), "'file[123]'");
        
        // Handle single quotes in path (use double quotes)
        assert_eq!(builtin.quote_path_if_needed("file's name"), "\"file's name\"");
        
        // Handle both single and double quotes (escape double quotes)
        assert_eq!(builtin.quote_path_if_needed("file's \"quoted\" name"), "\"file's \\\"quoted\\\" name\"");
    }
    ...
    fn test_strict_patterns() {
        ...
        assert!(builtin.match_command("list all files in directory").is_none()); // Too complex
